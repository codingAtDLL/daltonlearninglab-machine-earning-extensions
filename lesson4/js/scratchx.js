...
function JSshowImageDialog() {
    showModal(["template-image-file"]);
}
...
function sendURLtoFlash() {
    ...
    console.log(window.extensionURLs);
    if (Array.isArray(window.extensionURLs)) {
        urls = urls.concat(window.extensionURLs);
    }
    console.log("extensions: " + urls.length);
    if (urls.length <= 0) return;
    if (Scratch.editorIsReady) {
        $(document).trigger("editor:extensionLoaded", {method: "url", urls: urls});
        showPage(editorId);
        window.asobjReadyInterval = setInterval(function(){ 
            if (Scratch.FlashApp.ASobj != undefined) 
            {
                Scratch.FlashApp.ASobj.ASloadGithubURL(urls);
                clearInterval(asobjReadyInterval);
            }
        }, 500);
    } else {
        $(document).on("editor:ready",  function(e) {
...
function loadFromURLParameter(queryString) {
    ...
    for (var i=0; i<vars.length; i++) {
        ...
        else if (pair.length > 1 && pair[0]=="step") {
            tutorial_step = window.tutorial_step = parseInt(pair[1],10);
            if (tutorial_step > 3) {
                window.extensionURLs = ["step"+tutorial_step+".sbx"];
            }
            //setTimeout(function(){ showModal(["tutorial-step"+window.tutorial_step]);}, 000);
        }
    }
    if (tutorial_step == 0) {
    ...
    //if (urls.length > 0) {
        // manually delay the sending of URL
        //setTimeout(function(){ sendURLtoFlash.apply(window, urls); }, 5000);
    //}
...
$(document).on('click', "[data-action='next-step']", function(e) {
    ...
    window.tutorial_step++;
    showModal(["tutorial-step"+window.tutorial_step]);

});

$(document).on('click', "[data-action='close-dialog']", function(e) {
    $(document).trigger("modal:exit");
});

$(document).on('click', "[data-action='well-done']", function(e) {
    $(document).trigger("modal:exit");
    window.tutorial_step++;
...
$(document).on('click', "[data-action='load-image']", function(e) {
    /*
    Buttons with data-action="load-image" trigger a file input
    prompt, passed to a handler that passes the file to Flash.
    */
    $('<input type="file" />').on('change', function(){
        window.processUserImage(this.files);
        $(document).trigger("modal:exit")
    }).click();
});
