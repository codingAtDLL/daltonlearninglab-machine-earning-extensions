...
var Neuroevolution = function(options){
    ...
    self.ELITISM_BY_RATE = "RATE";
    self.ELITISM_BY_COUNT = "COUNT",
     ...
     */
        ...
        elitismRate:0,            // Best networks kepts unchanged for the next
        elitismCount:0,            // Best networks kepts unchanged for the next

        elitismMode:self.ELITISM_BY_COUNT,            // elitism mode
                        // generation (rate).
        randomBehaviour:0.2,    // New random networks for the next generation
                        // (rate).
        mutationRate:0,       // Mutation rate on the weights of synapses.
        mutationRange:0,      // Interval of the mutation changes on the
                        // synapse weight.
...
/*GENERATION******************************************************************/
    ...
    Generation.prototype.generateNextGeneration = function(){
        ...
        for(var i = 0; i < (self.options.elitismMode==self.ELITISM_BY_COUNT? self.options.elitismCount :Math.round(self.options.elitism
...
/*SELF************************************************************************/
    ...
    window.restartNeuralNetwork = function() {
        window.gen = [];
        window.Neuvol = new Neuroevolution({
                population:50,
                network:[2, [2], 1],
            });
    }
    window.restartNeuralNetwork();
    ...
    ext.setPopulation = function(populationCount) {
        window.Neuvol.options.population = populationCount;
    }

    ext.setElitismRate = function(elitismRate) {
        window.Neuvol.options.elitismMode = window.Neuvol.ELITISM_BY_RATE;
        window.Neuvol.options.elitismRate = elitismRate;        
    };

    ext.setElitismCount = function(elitismCount) {
        window.Neuvol.options.elitismMode = window.Neuvol.ELITISM_BY_COUNT;
        window.Neuvol.options.elitismCount = elitismCount;        
    };

    ext.setNetwork = function(networkString) {
        var networkArray = (new Function("return " + networkString+ ";")());
        if (Array.isArray(networkArray)) {
            window.Neuvol.options.network = networkArray;
        }
    }

    ext.setMutationRate = function(mutationRate) {
        window.Neuvol.options.mutationRate = mutationRate;
    }

    ext.setMutationRange = function(mutationRange) {
        window.Neuvol.options.mutationRange = mutationRange;
    }

    ext.setRandomBehaviourRate = function(rate) {
        window.Neuvol.options.randomBehaviour = rate;
    }

    ext.compute = function(i, inputs) {
        var inputArray = (new Function("return " + inputs+ ";")());
        if (Array.isArray(inputArray)) {
            return window.gen[i].compute(inputArray);
        } 
        return 0;
    }

    ext.setNetworkScore = function(i, score) {
        window.Neuvol.networkScore(window.gen[i], score);
    }

    ext.nextGeneration = function() {
        window.gen = window.Neuvol.nextGeneration();
    }  

    ext.restartNeuralNetwork = function() {
        window.restartNeuralNetwork();
    } 

    ext.nextTutorialStep = function(callback) {
        //window.tutorial_step += 1;
        //window.location.href = './?step='+window.tutorial_step;
        showModal(["tutorial-welldone"]);
    }    

    var descriptor = {
        blocks: [
            [' ', 'set population %n', 'setPopulation', 10],
            [' ', 'set elitism rate %n', 'setElitismRate', 0.2],
            [' ', 'set elitism count %n', 'setElitismCount', 1],
            [' ', 'set network structure %s', 'setNetwork', '[2, [2], 1]'],
            [' ', 'set mutation rate %n', 'setMutationRate', 0.1],
            [' ', 'set mutation range %n', 'setMutationRange', 0.5],
            [' ', 'set random behaviour rate %n', 'setRandomBehaviourRate', 0.2],
            ['r', 'generation[%n ] compute inputs %s', 'compute', 1, '[1,1]'],
            [' ', 'set network[%n ] with score: %n', 'setNetworkScore', '1', '0'],
            [' ', 'generate next generation', 'nextGeneration'],
            [' ', 'restart neural network', 'restartNeuralNetwork'],
            ['w', 'next tutorial step', 'nextTutorialStep']
        ]
    };

    ScratchExtensions.register('Dalton-AI-Lab-FlappyML', descriptor, ext);
